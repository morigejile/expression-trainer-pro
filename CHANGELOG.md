...
- 自动保存最近 50 条训练逐字稿、统计和报告；历史窗口使用紧凑列表，并可分别查看原文或已生成的报告。
- 主窗口加载完成后后台预热当前 ASR 模型，首次录制复用同一初始化任务，减少点击后的等待。
- 增加空格键开始、暂停和继续录制，并在输入区或弹窗打开时安全忽略快捷键。
- 报告首行改为由同一次大模型请求生成与本次表达相关的个性化鼓励彩蛋。
...
- 将 ASR 模型迁移到 `%APPDATA%\expression-trainer-pro-models`，避开 Windows native Sherpa 无法读取本地化 `userData` 路径的问题；诊断同步报告实际生效模型。
- 主窗口提示支持显式关闭，成功保存 LLM 配置后自动关闭配置相关旧提示；Windows/Linux 移除重复的窗口内嵌菜单栏。
