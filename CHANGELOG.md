...
- 增加显式 Internal Only 的 Zipformer Large 包内默认构建与离线导入资格路径；内部安装包使用 `ExpressionTrainerInternalOnly` 名称且只接受 Catalog 固定归档，普通制品验收会拒绝任何包内 ASR 模型；同时修复只读模型状态查询创建空 `models` 目录的回归。
