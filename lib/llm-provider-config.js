const CURRENT_LLM_PROVIDER_SCHEMA_VERSION = 1;
...
function createDefaultLlmProviderSettings() {
  return {
    schemaVersion: CURRENT_LLM_PROVIDER_SCHEMA_VERSION,
    provider: 'deepseek',
    providers: Object.fromEntries(
      Object.entries(DEFAULT_LLM_PROVIDER_CONFIGS).map(([provider, config]) => [
        provider,
        { ...config }
      ])
    )
...
function normalizeCustomConfig(config) {
  const normalized = { ...DEFAULT_LLM_PROVIDER_CONFIGS.custom, ...config };
...
function migrateLegacySettings(raw) {
  ...
  const provider = Object.hasOwn(DEFAULT_LLM_PROVIDER_CONFIGS, raw.provider)
    ? raw.provider
    : 'deepseek';
...
function normalizeLlmProviderSettings(raw) {
  ...
  const settings = createDefaultLlmProviderSettings();
  settings.provider = Object.hasOwn(DEFAULT_LLM_PROVIDER_CONFIGS, raw.provider)
    ? raw.provider
    : 'deepseek';

  for (const [provider, config] of Object.entries(raw.providers)) {
    if (!Object.hasOwn(DEFAULT_LLM_PROVIDER_CONFIGS, provider) && isRecord(config)) {
  ...
  for (const provider of Object.keys(DEFAULT_LLM_PROVIDER_CONFIGS)) {
    ...
    settings.providers[provider] = provider === 'custom'
      ...
      : { ...DEFAULT_LLM_PROVIDER_CONFIGS[provider], ...config };
  }
...
function needsMigration(raw) {
  if (isRecord(raw) && Number.isInteger(raw.schemaVersion) && raw.schemaVersion > CURRENT_LLM_PROVIDER_SCHEMA_VERSION) {
    return false;
  }
  if (!isRecord(raw) || raw.schemaVersion !== CURRENT_LLM_PROVIDER_SCHEMA_VERSION) {
    return true;
  }
  if (!isRecord(raw.providers) || !Object.hasOwn(DEFAULT_LLM_PROVIDER_CONFIGS, raw.provider)) {
    return true;
  }

  return Object.entries(DEFAULT_LLM_PROVIDER_CONFIGS).some(([provider, defaults]) => {
...
function parseLlmProviderSettingsJson(json) {
  ...
  try {
    ...
    return {
      settings: createDefaultLlmProviderSettings(),
      shouldPersist: false,
  ...
  return {
    settings: normalizeLlmProviderSettings(raw),
