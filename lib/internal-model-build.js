...
function assertArchiveOutsideProject({archivePath, projectRoot} = {}) {
  ...
  if (typeof projectRoot !== 'string' || !path.isAbsolute(projectRoot)) {
    throw new Error('Application source root must be absolute');
  }
  const relative = path.relative(path.resolve(projectRoot), path.resolve(archivePath));
  if (relative === '' || (!relative.startsWith(`..${path.sep}`) && !path.isAbsolute(relative))) {
    throw new Error('Internal model archive must be outside the application source tree');
  }
}
...
module.exports = {assertArchiveOutsideProject, stageInternalModelArchive, verifyInternalModelResourceTree};
