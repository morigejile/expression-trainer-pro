...
function activeModelSummary(modelRoot, modelId) {
  const fallback = {id: modelId, version: null, status: 'not-installed'};
  const pointerPath = path.join(modelRoot, 'active', `${modelId}.json`);
  try {
...
function createDiagnosticSnapshot({
  ...
  if (typeof modelRoot !== 'string' || !path.isAbsolute(modelRoot)) {
    throw new TypeError('modelRoot must be absolute');
  }
  boundedString(modelId, 'modelId', /^[a-z0-9][a-z0-9._-]{0,127}$/);
  ...
  return {
    ...
    asr: {
      model: activeModelSummary(modelRoot, modelId),
