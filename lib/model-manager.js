...
async function verifyModelDirectory(model, modelPath) {
...
async function extractTarBz2({archivePath, destination, rootDirectory, files}) {
  const listed = await execFileAsync('tar', ['-tjf', archivePath], {maxBuffer: 16 * 1024 * 1024, windowsHide: true});
  ...
  await execFileAsync('tar', ['-xjf', archivePath, '-C', destination, ...selectedEntries], {windowsHide: true});
}

async function writeResponseBody(response, filePath, maximumBytes) {
  ...
  const limiter = new Transform({
    transform(chunk, encoding, callback) {
      ...
      if (receivedBytes > maximumBytes) {
        callback(new Error(`Model archive exceeds registered byte size: ${maximumBytes}`));
        return;
      }
      callback(null, chunk);
    }
  });
  await pipeline(input, limiter, fs.createWriteStream(filePath, {flags: 'wx'}));
}
...
function createModelManager({
  ...
  async function activate(id, version) {
    ...
    try {
      ...
      if (previous?.schemaVersion === 1 && previous.modelId === model.id && typeof previous.version === 'string' && previous.version !== model.version) {
  ...
  async function rollback(id) {
    ...
    if (!active) throw new Error(`Model is not active: ${id}`);
    if (!active.previousVersion) throw new Error(`Model has no previous version: ${id}`);
    return activate(id, active.previousVersion);
  }

  async function install(id, {activate: shouldActivate = false, signal} = {}) {
    await prepareRoots();
    ...
    if (fs.existsSync(finalPath)) {
      const files = await verifyModelDirectory(model, finalPath);
    ...
    try {
      if (signal?.aborted) throw signal.reason || new DOMException('Model download aborted', 'AbortError');
      const archivePath = path.join(operationRoot, 'model.tar.bz2');
      const response = await fetchImpl(model.archive.url, {signal});
      if (!response || response.ok !== true) throw new Error(`Model download failed${response ? `: HTTP ${response.status}` : ''}`);
      const contentLength = response.headers?.get?.('content-length');
      if (contentLength !== null && contentLength !== undefined && Number(contentLength) !== model.archive.bytes) {
        throw new Error(`Model archive byte-size mismatch: expected ${model.archive.bytes}, got ${contentLength}`);
      }
      await writeResponseBody(response, archivePath, model.archive.bytes);
      ...
      await extractArchive({archivePath, destination: extractionRoot, format: model.archive.format, rootDirectory: model.archive.rootDirectory, files: model.files});
      const extractedModelPath = path.join(extractionRoot, model.archive.rootDirectory);
      const files = await verifyModelDirectory(model, extractedModelPath);
  ...
  return {activate, getActive, install, modelsRoot, rollback};
}
