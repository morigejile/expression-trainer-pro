...
async function writeResponseBody(response, filePath, maximumBytes, signal) {
  ...
  await pipeline(input, limiter, fs.createWriteStream(filePath, {flags: 'wx'}), {signal});
}
...
function createModelManager({
  ...
  async function installUnlocked(id, {activate: shouldActivate = false, signal} = {}) {
    ...
    try {
      ...
      await writeResponseBody(response, archivePath, model.archive.bytes, signal);
      throwIfAborted(signal);
