...
function resolveManagedAsrOptions(argv, catalogInput) {
  ...
  const userDataPath = oneValue(argv, '--user-data-path', 'userData path');
  const modelRoot = oneValue(argv, '--model-root', 'model root');
  if (!path.isAbsolute(userDataPath)) throw new Error('Managed ASR userData path must be absolute');
  if (!path.isAbsolute(modelRoot)) throw new Error('Managed ASR model root must be absolute');
