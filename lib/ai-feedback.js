...
const { getRealtimePrompt, getReportPrompt, getPlaybackAnalysisPrompt } = require('./prompts');
const {parsePlaybackAnalysisResponse} = require('./playback-analysis');
...
const ERROR_MESSAGES = {
  ...
  missingModel: '请先配置模型名称',
  invalidEndpoint: '大模型接口地址格式无效',
  invalidProvider: '大模型配置不受支持',
...
function toSafeErrorResult(error) {
  return error instanceof LLMRequestError
    ...
    : { error: ERROR_MESSAGES.generic, errorCode: 'generic' };
}
...
async function runCoordinatedRequest(
  ...
  try {
    ...
    return { success: false, ...toSafeErrorResult(error) };
  } finally {
    request.finish();
  }
}
...
async function callAPI(endpoint, apiKey, model, messages, maxTokens = 200, options = {}) {
  ...
  if (!model || !model.trim()) {
    throw new LLMRequestError('missing-model', ERROR_MESSAGES.missingModel);
  }
...
function getProviderConfig(settings) {
  ...
  switch (provider) {
    ...
    case 'ollama':
      return {
        endpoint: appendChatCompletions(ollamaUrl || 'http://localhost:11434', '/v1/chat/completions'),
    ...
    case 'custom':
      ...
      return {
        endpoint: baseUrl ? appendChatCompletions(baseUrl, '/chat/completions') : '',
        apiKey: apiKey || '',
        model: customModel || model || ''
      };
    default:
      throw new LLMRequestError('invalid-provider', ERROR_MESSAGES.invalidProvider);
  }
}

function appendChatCompletions(rawBaseUrl, suffix) {
  let url;
  try {
    url = new URL(rawBaseUrl);
  } catch {
    throw new LLMRequestError('invalid-endpoint', ERROR_MESSAGES.invalidEndpoint);
  }
...
async function sendPlaybackAnalysis(segments, profile, customPrompt, options = {}) {
  const config = getProviderConfig(profile);
  const prompt = getPlaybackAnalysisPrompt(segments, customPrompt);
  const raw = await callAPI(config.endpoint, config.apiKey, config.model, [
    {role: 'system', content: prompt.system},
    {role: 'user', content: prompt.user}
  ], 4096, {
    ...options,
    temperature: 0.2,
    timeoutMs: options.timeoutMs ?? REQUEST_TIMEOUTS.playback
  });
  try {
    return parsePlaybackAnalysisResponse(raw, new Set(segments.map(segment => segment.id)));
  } catch (error) {
