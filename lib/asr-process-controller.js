...
function createAsrProcessController({
  ...
  let lastInitializationElapsedMs = null;
  ...
  function handleExit(target, code) {
    ...
    if (child === target) {
      ...
      initialized = false;
      lastExitCode = code;
      lastErrorCategory = 'asr-process-exited';
    }
  }
  ...
  const controller = {
    initialize() {
      ...
      const startedAt = now();
      initializePromise = sendRequest('initialize', null, target, initializeTimeoutMs)
        .then(() => {
          if (child !== target) {
          ...
          initialized = true;
          lastInitializationElapsedMs = Math.max(0, now() - startedAt);
          lastErrorCategory = null;
        })
        .catch(error => {
          lastInitializationElapsedMs = Math.max(0, now() - startedAt);
          lastErrorCategory = typeof error?.code === 'string' ? error.code : 'asr-initialization-failed';
          throw error;
        })
