...
> 源码基线：`https://github.com/fxy2311-youyou/expression-trainer.git`，本地 `main@f925434`（`package-lock.json` 有未提交修改）
...
| NFR-02 | 可复现性 | 锁文件与 `package.json` 一致；干净环境使用 `npm ci` 后可执行约定的检查、测试和构建。当前 lock/安装树为 Electron 33.4.11、Sherpa 1.13.3；项目 Node/npm 版本仍需固定。 |
...
1. 当前 `package.json` 为应用 `1.0.0`、Electron `^33.0.0`、sherpa-onnx-node `^1.10.0`；lock/安装树为 33.4.11/1.13.3，项目 Node/npm 版本仍需干净构建记录。
2. README 声明 macOS 12+、Windows 10+、Linux，但没有 CI/制品证据；正式支持等级待定。
