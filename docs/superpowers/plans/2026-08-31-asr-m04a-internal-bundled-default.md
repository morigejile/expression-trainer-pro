...
- [x] **Step 4: Run the real internal artifact qualification when the external archive is available**
  ...
  Final independent review closed the remaining artifact-boundary gaps: the 402-test suite passed with 400 pass, 0 fail, and 2 host symlink skips; ordinary packaging keeps only `models/registry.json`, ordinary packaged smoke rejects any `resources/asr-models`, and the internal build revalidates an exact one-file Catalog-default resource tree before Forge runs. The internal Squirrel identity is `ExpressionTrainerInternalOnly` / `ExpressionTrainerInternalOnlySetup.exe`. Ordinary make and packaged smoke passed; the same ordinary check rejected the internal artifact as intended. Internal make and bundled-default smoke passed with 17.156 seconds for first offline import/native initialization and 2.479 seconds for second offline startup. This final pass still used Node 24.19.0/npm 12.0.2, so the declared baseline rerun remains required before PR.

  A second review found the remaining workspace-path bypass. The final boundary now requires the source archive outside the application source tree, globally excludes supported model weight/archive suffixes from the ordinary ASAR input, and inspects the completed ASAR manifest in ordinary smoke. The final 404-test suite passed with 402 pass, 0 fail, and 2 host symlink skips. Ordinary make/smoke passed, ordinary smoke rejected the final internal artifact, and the internal bundled-default smoke passed at 17.398 seconds first import/native initialization and 2.510 seconds second startup.
