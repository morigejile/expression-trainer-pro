...
### Task 2: Hash-pinned model attempts and prediction evidence
...
**Interfaces:**
- Consumes Task 1 `binding` and `resolveContained`.
- Produces `validateModelLock(lock): ModelLock`, `verifyModelRole({ modelRoot, role }): VerifiedRole`, `buildReviewSherpaConfig(role, modelRoot): object`, and `sealPredictionAttempt({ binding, role, modelLock, modelRoot, runId, transcribe }): AttemptRecord`.
...
- [ ] **Step 1: Write the failing test**
  Use fake files/injected `transcribe`. Assert exactly `baseline-paraformer`, `candidate-zipformer`, `candidate-sensevoice-small`; relative model paths; hash mismatch rejection; redacted model-root errors; one success; and one sealed failure with no false text.
- [ ] **Step 2: Run test to verify it fails**
  Run: `$env:Path = 'C:\Users\mr\AppData\Local\hermes\node;' + $env:Path; node --test test/assisted-review-models.test.js`
  Expected: FAIL because the model module does not exist.
- [ ] **Step 3: Write minimal implementation**
  Port only `canonicalizeModelRoot`, `resolveModelPath`, SHA verification, and family config construction from model-prep. Require all three roles; revalidate bound PCM before/after each attempt; create new JSON below `runs/<run-id>/candidates/<id>/<binding>/predictions/`; do not alter production ASR.
- [ ] **Step 4: Run test to verify it passes**
  Run: `$env:Path = 'C:\Users\mr\AppData\Local\hermes\node;' + $env:Path; node --test test/assisted-review-models.test.js`
  Expected: PASS with sealed outcomes and no absolute model root in evidence.
- [ ] **Step 5: Commit**
...
### Task 3: Unicode CER, medoid, and risk evidence
...
**Interfaces:**
- Consumes Task 1 PCM and Task 3 comparison.
...
- Human actions are `record-primary-transcript`, `approve-secondary-transcript`, `approve-license`, `clear-pii`, `set-final-tags`, `approve-policy`; all require `{ actorAlias, actorRole, bindingSha256 }`.

- [ ] **Step 1: Write the failing test**
  Assert alias regex, equal secondary alias rejection, ordered transition, stale revision, competing writers, prior-hash continuity, crash replay, broken-chain quarantine, and no approval transfer to a new chain.
- [ ] **Step 2: Run test to verify it fails**
  Run: `$env:Path = 'C:\Users\mr\AppData\Local\hermes\node;' + $env:Path; node --test test/assisted-review-audit.test.js`
  Expected: FAIL because the audit module does not exist.
- [ ] **Step 3: Write minimal implementation**
  Validate exact keys; lock per candidate; append/fsync canonical audit before fsync-plus-rename state; replay only a contiguous valid chain; preserve broken evidence and restart candidate `unreviewed`.
- [ ] **Step 4: Run test to verify it passes**
  Run: `$env:Path = 'C:\Users\mr\AppData\Local\hermes\node;' + $env:Path; node --test test/assisted-review-audit.test.js`
...
**Interfaces:**
- Consumes Tasks 1, 4, 5 through injected stores.
- Produces `createReviewServer({ datasetRoot, reviewStore, tokenBytes, port }): { url, close, server }`; routes: `GET /?token=`, `GET /review`, `GET /api/candidates/:id`, `GET /api/candidates/:id/audio`, `POST /api/candidates/:id/transitions`.

- [ ] **Step 1: Write the failing test**
  Start port zero; assert `127.0.0.1`, single-use 256-bit exchange/redirect, cookie/CSRF/origin checks, opaque IDs, hostile transcript JSON, headers, contained audio, and role-checked forwarding.
- [ ] **Step 2: Run test to verify it fails**
  Run: `$env:Path = 'C:\Users\mr\AppData\Local\hermes\node;' + $env:Path; node --test test/assisted-review-server.test.js`
  Expected: FAIL because the server module does not exist.
- [ ] **Step 3: Write minimal implementation**
  Bind only IPv4 loopback, exchange token for short-lived `HttpOnly; SameSite=Strict` cookie, reject malformed/oversized input, set CSP/no-referrer/no-cache/nosniff/frame headers, use `textContent` in UI, and use Task 1 opaque-ID audio rechecks.
- [ ] **Step 4: Run test to verify it passes**
  Run: `$env:Path = 'C:\Users\mr\AppData\Local\hermes\node;' + $env:Path; node --test test/assisted-review-server.test.js`
...
**Interfaces:**
- `parseAssistedReviewArgs(argv): { command, datasetRoot, modelRoot, modelLockPath, candidateIds, runId, exportId, dryRun }` rejects duplicate/unknown flags and repository-root outputs.
...
- [ ] **Step 1: Write the failing test**
  Spawn with fake dependencies; assert `dry-run` validates 100 candidate bindings without native inference, missing opt-in fails, duplicate flags fail, repository-root output fails, and normal `npm test` reads no external root/model.
- [ ] **Step 2: Run test to verify it fails**
...
- Spec coverage: Tasks 1–2 implement containment, bindings, three hash-pinned Sherpa attempts, immutable evidence; Task 3 implements normalisation/CER/medoid/risk; Task 4 implements PII/tag/SNR and policy approval; Task 5 implements roles/state/audit/recovery; Task 6 implements loopback/token/cookie/CSRF/safe rendering; Task 7 implements every export gate; Task 8 implements wiring, docs, checks, and opt-in 100-candidate run.
...
- Type consistency: Task 1 creates `bindingSha256`; Tasks 2, 5, 7 consume it unchanged. Task 2 creates `AttemptRecord`; Tasks 3 and 7 consume it. Task 4 creates `PolicyApproval`; Task 7 consumes it. Task 5 alone creates approval events.
