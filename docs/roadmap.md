...
> 更新日期：2026-08-31
> 当前进度：Phase 0-5 的内部基线、R-01～R-09、PKG-01～PKG-04、OPS-02/OPS-05、七候选 benchmark、CONV-01～CONV-03、ASR-M01～M03 与 ASR-M04a 内部资格验证已完成。完整 ASR-M04 仍受许可和公开制品条件约束。
...
→ UI-01/UI-02 与 ASR-M01～M03 分轨推进；ASR-M01～M03 已完成
...
| ASR-M03 | P1 | Completed | 独立安装任务、模型管理 IPC 和设置页模型区域 | ASR-M02 | 354 项全量测试中 352 pass、0 fail、2 个既有 Windows symlink skip；下载可取消重试，Renderer 仅提交受信任模型 ID，安装不影响当前识别 |
| ASR-M04a | P1 | Completed/Internal Only | 显式内部构建携带 Catalog 默认 Zipformer Large，并复用 ModelManager 完成包内离线导入与真实 native 初始化 | ASR-M02 | 与 ASR-M03 合并后 370 项中 368 pass、0 fail、2 个 Windows symlink skip；内部 Squirrel make 成功；打包应用首次离线启动 17.154 秒、二次 2.547 秒；归档不进入 Git |
...
R-01～R-09、D-03/D-04、PKG-01～PKG-04、BM-04、ASR-M01～M03 与 ASR-M04a 已完成当前最小边界。产品会恢复独立持久选择，无选择时采用 Zipformer Large 技术默认；设置页通过脱敏快照与四个受限命令管理下载、取消、重试和切换，安装任务独立于当前识别 controller；内部包已验证从应用资源离线导入至 `userData/models`。公开分发仍取决于许可与产品链路验收。
...
ADR-0009 已采用 Zipformer Large 作为技术默认；ASR-M02 已接入启动恢复，旧 Paraformer 安装继续保留；ASR-M03 已完成设置页切换入口与独立安装任务；ASR-M04a 已完成显式内部包的离线导入和真实初始化。公开升级验收、签名和再分发授权仍属于完整 ASR-M04。
...
| M7 新需求产品化 | In Progress | UI-01～UI-02、ASR-M01～ASR-M04（ASR-M01～M03、ASR-M04a Completed） | 三款 streaming Catalog/Factory、选择/单 controller 服务、安装设置入口与内部包内默认资格已完成；响应式外观与公开默认模型条件继续分轨交付 |
