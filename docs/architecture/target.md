...
当前 Paraformer 是对照基线。首轮候选仅包含：

- 小型中文 streaming Zipformer CTC；
- SenseVoiceSmall INT8（需明确其 utterance/VAD 使用方式与 streaming 模型的体验差异）。

较大中文 Zipformer、新模型和新语料源列为后续补充，不进入首轮候选矩阵。

不得把公开榜单或模型发布时间当作项目结论。统一 benchmark 需要：

- 经过授权且脱敏的 50～100 条真实中文表达训练音频（最终数量以数据可得性为准）；
- 首轮允许只覆盖一至两类已取得的语料特征；当前 FLEURS 普通话现状可接受并必须记录局限；
- 普通话、语速变化、轻口音、中英混合、数字/专名、安静与轻噪声的完整覆盖属于后期优化，不阻塞首轮选型；
- 固定硬件、线程数、warm/cold start、相同音频与标注；
- CER、首个 partial、最终延迟、RTF、CPU、峰值 RAM、模型大小、初始化时间；
- 识别质量之外同时评估安装体积、许可证、跨平台包和集成复杂度。

首轮基本门槛为 failure rate 不高于 5%、RTF 不高于 1。满足门槛后以 CER 为首要选型指标；CER 处于重复运行波动范围内时，性能和资源占用作为次级判断。Streaming UX 独立记录，不用虚构可比的 partial；许可证不阻塞内部测试，但在 D-02 选择发布默认模型时是硬门槛。规则必须在查看汇总结果前冻结。
