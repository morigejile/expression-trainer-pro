...
- ASR-M04a 已在与 ASR-M03、UI-01/UI-02 组合后完成 402 项测试（400 pass、0 fail、2 个 Windows file-symlink skip）、普通 model-free Squirrel make/packaged smoke、显式 `ExpressionTrainerInternalOnly` Squirrel make、全新隔离 `userData` 的首次离线导入/native 初始化（17.156 秒）及二次离线启动（2.479 秒）。内部资源树只允许 Catalog 固定默认归档，普通制品验收会拒绝任何包内 ASR 模型。模型归档不进入 Git；本轮使用 Codex host 的 Node 24.19.0/npm 12.0.2，正式 PR 前仍需在声明的 Node 24.20.0/npm 11.19.0 基线上复跑。公开安装、升级、签名和再分发许可仍归完整 ASR-M04。
