...
> 状态：PKG-03 first-install baseline
> 更新日期：2026-08-29
...
| 安装形态 | Electron Forge/Squirrel Windows x64 制品 | PKG-03 已执行静默安装、真实 Paraformer 准备/native 初始化、强制离线二次启动和清理；升级/卸载数据策略留给 PKG-04 |
...
| 等级 | 平台 | 当前含义 |
|---|---|---|
| Tier 1 target | Windows 11 25H2+ x64 | PKG-04 的唯一近期发布路径；未签名内部制品已有安装/真实模型证据，升级证据仍待完成 |
