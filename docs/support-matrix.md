...
| 安装形态 | Electron Forge/Squirrel Windows x64 制品 | PKG-02 已在干净依赖环境生成 Setup.exe、full.nupkg 与 RELEASES；packaged Fake/native-load smoke 通过，尚未执行安装和真实模型闭环 |
...
| 等级 | 平台 | 当前含义 |
|---|---|---|
| Tier 1 target | Windows 11 25H2+ x64 | PKG-03/PKG-04 的唯一关键路径；已有未签名内部制品，安装、真实模型和升级证据仍待完成 |
